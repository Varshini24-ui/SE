...
// Section definitions for robust parsing using regex (enhanced version imported from utils)
const RESUME_SECTIONS = ENHANCED_RESUME_SECTIONS;
...
function plainToHTML(resumeText) {
  ...
  lines.forEach((trimmed) => {
    ...
    const isBullet = isBulletPoint(trimmed);
    // Enhanced bullet removal - handles all supported formats
    let contentWithoutBullet = trimmed;
    if (isBullet) {
      contentWithoutBullet = trimmed.replace(/^[-*•→►✓>○■▪⋅]\s+/, "")
        .replace(/^\d+\.\s+/, "")
        .replace(/^\d+\)\s+/, "")
        .replace(/^\(\d+\)\s+/, "")
        .replace(/^[a-z]\.\s+/i, "")
    ...
    const safeContent = escapeHTML(contentWithoutBullet);
...
function calculateATSScore(resumeText, jobDescription) {
  ...
  Object.entries(RESUME_SECTIONS).forEach(([k, rgx]) => {
    ...
    if (isFound) {
      found[k] = true;
    } else if (CORE_SECTIONS.includes(k)) {
      missing.push(k);
    }
  });
...
export default function ResumeAnalyzer() {
  ...
  const handleFile = useCallback(
    (file) => {
      ...
      if (file.type === "application/pdf") {
        setFileError(
          "⚠️ PDF selected. Use .txt for accurate analysis. Dummy content loaded for demonstration.",
        );
        setResumeText(
  ...
  // Effect to extract structured data from resume
  useEffect(() => {
    if (!resumeText) {
      setExtractedData(null);
      return;
    ...
    const experienceSection = extractSectionContent(resumeText, "experience");
    const skillsSection = extractSectionContent(resumeText, "skills");
    const educationSection = extractSectionContent(resumeText, "education");

    const categorizedSkills = skillsSection ? categorizeSkills(skillsSection) : null;
    const totalExperience = experienceSection ? calculateTotalExperience(experienceSection) : null;
  ...
  const feedback = useMemo(() => {
    ...
    if (missing.length > 0) {
      list.push(
        `<span class="feedback-icon bad">✖</span> Structure Deficiency: Missing core sections: <strong>${missing.map((s) => s.toUpperCase()).join(", ")}</strong>. (Impact: High)`,
      );
    } else {
      list.push(
        '<span class="feedback-icon ok">✓</span> Structure Complete: All standard core sections found.',
      );
    }
    ...
    if (uniqueJD === 0) {
      list.push(
        '<span class="feedback-icon info">i</span> No JD provided. Paste one to calculate keyword alignment and boost your score!',
      );
    } else if (matchPct < 40) {
      list.push(
        `<span class="feedback-icon bad">#</span> Low Keyword Match (${matchPct}%): Major tailoring needed. Missing critical terms like: <strong>${missingKeys.slice(0, 3).join(", ")}...</strong>`,
      );
    } else if (matchPct < 70) {
      list.push(
        `<span class="feedback-icon warn">!</span> Moderate Match (${matchPct}%): Add missing terms for a stronger ATS score. You're close!`,
      );
    } else {
      list.push(
        `<span class="feedback-icon ok">✓</span> Strong Match (${matchPct}%): Excellent keyword alignment.`,
      );
    }
    ...
    if (weakWordCount > 0) {
      ...
      const suggestions =
        actionVerbs.length > 0 ? actionVerbs.join(", ") : "Spearheaded, Drove, Led";

      list.push(
        `<span class="feedback-icon warn">!</span> Vocabulary: Used weak verbs <strong>${weakWordCount}</strong> times. Replace passive terms like 'responsible for' with stronger **action verbs** like: <strong>${suggestions}</strong>.`,
      );
    } else {
      list.push(
        '<span class="feedback-icon ok">✓</span> Vocabulary: Strong action verbs detected.',
      );
    }

    // 4. Final Score
    list.push(
      `<span class="feedback-icon brand">#</span> Final ATS Score: <strong>${atsScore}%</strong>. Target 80%+ for top performance.`,
    );
  ...
  const templateRecommendation = useMemo(() => {
    ...
    if (atsScore >= 80) {
      return RESUME_TEMPLATES.modern.id;
    }
    if (atsScore >= 50) {
      return RESUME_TEMPLATES.minimal.id;
    }
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (!jsPDF) {
        throw new Error("jsPDF object not found after loading UMD bundle.");
      }
      ...
      alert(
        `PDF Generation failed. Please check the console for technical details. (Error: ${err.message})`,
      );
    } finally {
      ...
      <section className="container">
        <div className="grid-2">
          ...
          <div className="card">
            ...
            />
            <p className="hint">
              Pasting the JD is crucial. The tool analyzes keywords against this text for 30% of
              your ATS score.
            </p>
          </div>
        </div>
        ...
        {resumeText && (
          <>
              ...
              <div className="card ats-analysis-card">
                  ...
                  <div className="analysis-points">
                    ...
                    {templateRecommendation && selectedTemplate !== templateRecommendation && (
                      <p className="hint mt-05 template-recommendation">
                        **Template Tip:** Based on your score, the **
                        {RESUME_TEMPLATES[templateRecommendation].name}** template is recommended
                ...
                <div className="actions">
                  <button className="btn primary" onClick={handleDownload} disabled={isProcessing}>
                    {isProcessing
                      ? "Processing PDF..."
                      : `⬇️ Download '${currentTemplate.name}' as PDF`}
                  </button>
                  <p className="hint mt-05">
                    Generates a high-quality A4 PDF of your formatted resume.
                  </p>
                </div>
              </div>
            )}
            ...
            {extractedData && (
              <div className="card">
                <h2 className="section-title">📋 Extracted Resume Data</h2>

                {/* Contact Information */}
                {extractedData.contact && (
                  <div className="extracted-section">
                    <h3>Contact Information</h3>
                    {extractedData.contact.emails.length > 0 && (
                      <p><strong>Email:</strong> {extractedData.contact.emails.join(", ")}</p>
                    )}
                    {extractedData.contact.phones.length > 0 && (
                      <p><strong>Phone:</strong> {extractedData.contact.phones.join(", ")}</p>
                    )}
                    {extractedData.contact.linkedin.length > 0 && (
                      <p><strong>LinkedIn:</strong> {extractedData.contact.linkedin.join(", ")}</p>
                    )}
                    {extractedData.contact.github.length > 0 && (
                      <p><strong>GitHub:</strong> {extractedData.contact.github.join(", ")}</p>
                    )}
                    {extractedData.contact.portfolio.length > 0 && (
                      <p><strong>Portfolio:</strong> {extractedData.contact.portfolio.join(", ")}</p>
                    )}
                  </div>
                )}

                {/* Total Experience */}
                {extractedData.experience && extractedData.experience.totalYears >= 0 && (
                  <div className="extracted-section">
                    <h3>Total Experience</h3>
                    <p><strong>{extractedData.experience.formatted}</strong> ({extractedData.experience.ranges} position{extractedData.experience.ranges !== 1 ? "s" : ""})</p>
                  </div>
                )}
                ...
                {extractedData.skills && (
                  <div className="extracted-section">
                    ...
                    {extractedData.skills.programmingLanguages.length > 0 && (
                      <p><strong>Programming Languages:</strong> {extractedData.skills.programmingLanguages.join(", ")}</p>
                    )}
                    {extractedData.skills.frameworks.length > 0 && (
                      <p><strong>Frameworks:</strong> {extractedData.skills.frameworks.join(", ")}</p>
                    )}
                    {extractedData.skills.databases.length > 0 && (
                      <p><strong>Databases:</strong> {extractedData.skills.databases.join(", ")}</p>
                    )}
                    {extractedData.skills.tools.length > 0 && (
                      <p><strong>Tools:</strong> {extractedData.skills.tools.join(", ")}</p>
                    )}
                    {extractedData.skills.softSkills.length > 0 && (
                      <p><strong>Soft Skills:</strong> {extractedData.skills.softSkills.join(", ")}</p>
                    )}
                    {extractedData.skills.other.length > 0 && (
                      <p><strong>Other:</strong> {extractedData.skills.other.join(", ")}</p>
                    )}
                  </div>
                )}
              </div>
            ...
            <div
              ...
              style={{ fontFamily: currentTemplate.fontStack }}
            >
      ...
      <div className={`chatbot-container ${isChatbotOpen ? "open" : ""}`}>
        <div className="chatbot">
          <div className="chat-header">
            ...
            <button className="x" onClick={() => setIsChatbotOpen(false)} title="Close Chat">
              ✕
            </button>
          </div>
          <div className="chat-messages">
            <div className="chat-message bot">
              Hello! I can help you with your resume analysis. Ask me about your score or missing
              keywords!
            </div>
          </div>
